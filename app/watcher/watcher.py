...
def check_once(config: dict[str, Any], state: dict[str, Any]) -> int:
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            try:
                ...
                print(f"[{timestamp()}] page network is idle")
    ...
    if not baseline_done:
        ...
        print(f"[{timestamp()}] initial scan complete; matches were not saved")
...
def main() -> int:
    ...
    try:
        ...
        if args.test_notify:
            notify(
                config,
                "Skins availability watcher test",
        ...
        while True:
            try:
                ...
                print(f"[{timestamp()}] check complete, notifications sent: {new_count}")
